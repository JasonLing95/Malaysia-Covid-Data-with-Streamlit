...
vaccine_dos = state_vaccine[['state', 'cumul_partial', 'cumul_full', 'date']].sort_values('date').groupby('state').tail(1)
vaccine_dos['dose1_percent'] =  vaccine_dos.apply(lambda x: change_percentage(x['state'], x['cumul_partial']), axis=1)
vaccine_dos['dose2_percent'] =  vaccine_dos.apply(lambda x: change_percentage(x['state'], x['cumul_full']), axis=1)
